...
void GlobalServer::setupGlobalServer()
{
    ...
    std::cout << GREEN + getTimeOfDay() + " [ok] : Created server(s) sucessfully!\n" DEFAULT << std::endl;
}

void GlobalServer::loopServer()
{
    std::cout << BLUE + getTimeOfDay() + " [info] : Waiting events...\n" DEFAULT << std::endl;
...
void GlobalServer::handleNewClientConnection(int &serverFd)
{
    ...
    while ((clientSocket = accept(serverFd, (struct sockaddr *)&clientAddr, &clientAddrSize)) != -1)
    {
        ...
        std::cout << MAGENTA + getTimeOfDay() + " [connection] : Accepted new connection from port ‘" << ntohs(clientAddr.sin_port) << "’ (as fd " << clientSocket << ")" DEFAULT << std::endl;
    }
}
...
void GlobalServer::handleReading(ClientContext *clientContext)
{
    ...
    // std::cout << "\"" << clientContext->recvBuffer << "\"" << std::endl; // DEBUG
    ...
    if (clientContext->state == READY_TO_SEND)
    {
        try
        {
            ...
            // std::cout << YELLOW "Response generated by HTTPRequest()" DEFAULT << std::endl; // DEBUG
